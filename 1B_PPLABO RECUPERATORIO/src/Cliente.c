...
int cliente_alta(eCliente listaClientes[], int lenClientes, int* refId, eLocalidad listaLocalidades[], int lenLocalidades, eZona listaZonas[], int lenZonas, int* refIdLocalidad)
{
	int retorno = ERROR;
	int index;
	...
	if(listaClientes != NULL && lenClientes > 0 && refId != NULL && listaLocalidades != NULL && lenLocalidades > 0)
	{
		if(cliente_buscarLibre(listaClientes, lenClientes, &index) == FUNCIONO)
		{
			...
			{
				if(localidad_mostrarLista(listaLocalidades, lenLocalidades, listaZonas, lenZonas) == FUNCIONO &&
				...
				//if(localidad_alta(listaLocalidades, lenLocalidades, listaZonas, lenZonas, *refIdLocalidad) == FUNCIONO) ESTO SI QUIERO DAR DE ALTA LA LOCALIDAD.
				{
					//listaClientes[index].idLocalidad = *refIdLocalidad; ESTO SI QUIERO DAR DE ALTA LA LOCALIDAD.
	...
	if(retorno == FUNCIONO)
	{
		printf("\nSe dio de alta satisfactoriamente con el ID = %d", listaClientes[index].idCliente);
	}
	return retorno;
}
...
int cliente_modificar(eCliente listaClientes[], int lenClientes, eLocalidad listaLocalidades[], int lenLocalidades, eZona listaZonas[], int lenZonas)
{
	int retorno = ERROR;
	int opcion;
	int auxId;
	...
	if(cliente_mostrarListado(listaClientes, lenClientes, listaLocalidades, lenLocalidades) == FUNCIONO)
	{
		   ...
		   cliente_buscarPorId(listaClientes, lenClientes, auxId, &index) == FUNCIONO)
		{
			...
			if(cliente_modificarSegunOpcion(listaClientes, lenClientes, listaLocalidades, lenLocalidades, listaZonas, lenZonas, index, opcion) == FUNCIONO)
			{
				retorno = FUNCIONO;
			}
		}
...
int cliente_modificarSegunOpcion(eCliente listaClientes[], int lenClientes, eLocalidad listaLocalidades[], int lenLocalidades, eZona listaZonas[], int lenZonas, int auxIndex, int auxOpcion)
{
	int retorno = ERROR;
	...
	if(listaClientes != NULL && lenClientes > 0 && listaLocalidades != NULL && lenLocalidades)
	{
		...
		case AMBOS:
			if(Utn_GetString(auxDireccion, LEN_CADENA, "\nIngrese la nueva dirección: ", MENSAJE_ERROR, REINTENTOS) == FUNCIONO &&
			   ResetString(auxDireccion) == FUNCIONO &&
			   localidad_mostrarLista(listaLocalidades, lenLocalidades, listaZonas, lenZonas) == FUNCIONO &&
			   Utn_GetInt(&listaClientes[auxIndex].idLocalidad, "\nIngrese el ID de la nueva localidad: ", MENSAJE_ERROR, MIN_LOCALIDAD, MAX_LOCALIDAD, REINTENTOS) == FUNCIONO)
			{
				strcpy(listaClientes[auxIndex].direccion, auxDireccion);
				retorno = FUNCIONO;
