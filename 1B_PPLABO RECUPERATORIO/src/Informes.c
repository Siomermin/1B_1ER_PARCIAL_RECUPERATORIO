...
int informe_promedioKilosPolipropeno(ePedido listaPedidos[], int lenPedidos, eCliente listaClientes[], int lenClientes, ePago listaPagos[], int lenPagos)
{
	...
	if(contadorPedidos > 0)
	{
		...
		printf("\nLos clientes reciclaron: %2.2f kilos de PP en promedio!", promedioPP);
		retorno = FUNCIONO;
	}
	return retorno;
}
...
int promedio_pagosLiquidadosPorZona(eCliente listaClientes[], int lenClientes, ePedido listaPedidos[], int lenPedidos, eLocalidad listaLocalidades[], int lenLocalidades, ePago listaPagos[], int lenPagos, eZona listaZonas[], int lenZonas)
{
	...
	if(listaClientes != NULL && lenClientes > 0 && listaPedidos != NULL && lenPedidos > 0 && listaLocalidades != NULL && lenLocalidades > 0 && listaPagos != NULL && lenPagos > 0)
	{
		for(i=0;i<lenClientes;i++)
		{
			if(listaClientes[i].isEmpty == OCUPADO)
			{
				if(informe_contadorClientePorZona(listaLocalidades, lenLocalidades, listaClientes[i], listaZonas, lenZonas, &contadorPorZona) == FUNCIONO)
				{
					if(informe_pedidoClientePorZona(listaPedidos, lenPedidos, listaClientes[i], &indexPedido) == FUNCIONO)
					{
						if(informe_acumuladorPagosPorZona(listaPagos, lenPagos, listaPedidos[indexPedido], &acumuladorDineroLiquidado) == FUNCIONO)
						{
							retorno = FUNCIONO;
						}
					}
	...
	if(contadorPorZona > 0)
	{
		promedio = acumuladorDineroLiquidado / contadorPorZona;
		printf("\nEl promedio de pagos liquidados es de : $%2.2f", promedio);
	}
	return retorno;
}
...
int informe_contadorClientePorZona(eLocalidad listaLocalidades[], int lenLocalidades, eCliente unCliente, eZona listaZonas[], int lenZonas, int* contadorLocalidades)
{
	...
	if(listaLocalidades != NULL && lenLocalidades > 0)
	{
		if(zona_mostrarLista(listaZonas, lenZonas) == FUNCIONO &&
		   Utn_GetInt(&idZona,"Ingrese el ID de la zona a calcular promedio: ", MENSAJE_ERROR, MIN_ZONA, MAX_ZONA, REINTENTOS) == FUNCIONO)
		{
			for(i=0;i<lenLocalidades;i++)
			{
				if(listaLocalidades[i].idZona == idZona && listaLocalidades[i].isEmpty == OCUPADO && listaLocalidades[i].idLocalidad == unCliente.idLocalidad)
				{
					(*contadorLocalidades)++;
					retorno = FUNCIONO;
				}
